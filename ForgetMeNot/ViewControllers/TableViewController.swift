...
class TableViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if offset < -150 {
            //tableView.refreshControl?.attributedTitle = NSAttributedString(string: "불러오기가 완료되었습니다. 이제 놓으셔도 됩니다 :)", attributes: [NSAttributedStringKey.foregroundColor : refreshControl?.tintColor])
        } else {
            //tableView.refreshControl?.attributedTitle = NSAttributedString(string: "Fetcing More Data...", attributes: [NSAttributedStringKey.foregroundColor : refreshControl?.tintColor])
        }
        tableView.refreshControl?.backgroundColor = UIColor.darkGray
    }
    @objc
